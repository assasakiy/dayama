...
@php
    $displayCategory = $currentCategory ?? $post->primaryCategory;
@endphp
...
<article class="group flex flex-col h-full gap-3 {{ $attributes->get('class') }}">
    ...
    <div class="flex items-center gap-2">
        <a href="{{ route('category.show', $displayCategory) }}" class="chip text-xs no-underline hover:bg-surface-muted transition-colors">
            {{ $displayCategory->name }}
        </a>
        <span class="text-xs text-muted-foreground">{{ $post->published_at?->diffForHumans() ?? $post->created_at->diffForHumans() }}</span>
    </div>
    @endif
    ...
    <div class="flex items-center gap-3 text-xs text-muted-foreground mt-auto pt-2 border-t border-border-subtle">
        ...
        <a href="{{ route('author.show', $post->author) }}" class="flex items-center gap-1.5 hover:text-foreground transition-colors no-underline">
            <img src="{{ $post->author->avatar_url }}" alt="" class="w-5 h-5 rounded-full bg-surface-muted object-cover" loading="lazy">
