<?php
    $context = request()->getHost() === config('app.domains.blog') ? 'blog' : 'landing';
...
<xsl:variable name="site-name" select="'{{ $siteName }}'"/>
...
<div class="container">
    <xsl:variable name="section">
        <xsl:choose>
            <xsl:when test="contains($path, 'sitemap-profil')">profil</xsl:when>
            <xsl:when test="contains($path, 'sitemap-pendidikan')">pendidikan</xsl:when>
            <xsl:when test="contains($path, 'sitemap-layanan')">layanan</xsl:when>
            <xsl:when test="contains($path, 'sitemap-media')">media</xsl:when>
    ...
    <div class="nav-links">
        ...
        <a href="/sitemap-profil.xml"><xsl:if test="$section = 'profil'"><xsl:attribute name="class">active</xsl:attribute></xsl:if>Profil</a>
        <a href="/sitemap-pendidikan.xml"><xsl:if test="$section = 'pendidikan'"><xsl:attribute name="class">active</xsl:attribute></xsl:if>Pendidikan</a>
        <a href="/sitemap-layanan.xml"><xsl:if test="$section = 'layanan'"><xsl:attribute name="class">active</xsl:attribute></xsl:if>Layanan</a>
        <a href="/sitemap-media.xml"><xsl:if test="$section = 'media'"><xsl:attribute name="class">active</xsl:attribute></xsl:if>Media</a>
    </div>
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <div class="card-header">
                    <xsl:choose>
                        <xsl:when test="$section = 'profil'">Profil</xsl:when>
                        <xsl:when test="$section = 'pendidikan'">Pendidikan</xsl:when>
                        <xsl:when test="$section = 'layanan'">Layanan</xsl:when>
                        <xsl:when test="$section = 'media'">Media</xsl:when>
