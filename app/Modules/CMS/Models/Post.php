...
class Post extends \Illuminate\Database\Eloquent\Model implements HasMedia
{
    ...
    {
        \Illuminate\Support\Facades\DB::statement('UPDATE tags SET posts_count = (SELECT COUNT(*) FROM cms_post_tag WHERE cms_post_tag.tag_id = tags.id)');
        \Illuminate\Support\Facades\DB::statement('UPDATE categories SET posts_count = (SELECT COUNT(*) FROM cms_category_post WHERE cms_category_post.category_id = categories.id)');
    }
}
